...
from annotator_app.models import Annotation, Page
...
# http://docs.annotatorjs.org/en/latest/modules/storage.html
def getroot(request):
    ...
    if (isValid(reqBody, req.user, mode="update")):
        try:
            ...
            if (req.user.is_authenticated and annot.user == req.user.username):
                ...
                annot.tags = json.dumps(reqBody['tags'])
...
def apiCreate(req):
    ...
    if (isValid(reqBody, req.user)):
        ...
        try:
            ...
            if (int(page.revId) == int(reqBody['revision_id'])):
                newAnnot = Annotation()
                ...
                #prova tag
                newAnnot.tags = json.dumps(reqBody['tags'])
        ...
        return("Request not valid (bad fields cazzoo)")
